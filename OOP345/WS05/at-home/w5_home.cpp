...
int main(int argc, char** argv)
{
   ...
   sdds::Collection<sdds::Book> library("Bestsellers");
   {
      ...
      do {
         std::getline(file, strInfo);
         if (file) {
            if (strInfo[0] != '#') {
               library += Book(strInfo);
               cnt++;
            }
         }
      } while (file && cnt <= 3);
   ...
   Movie movies[5];
   {
      ...
      catch (std::out_of_range & msgs) {
         std::cout << "EXCEPTION: " << msgs.what() << std::endl;
      }
      std::cout << "-----------------------------------------\n\n";
      ...
      for (auto i = 3u; i <= 4u; ++i)
      {
         ...
         catch (const char* msgs) {
            std::cout << "EXCEPTION: " << msgs << std::endl;
         }
      }
      std::cout << "--------------- Movies ------------------\n";
