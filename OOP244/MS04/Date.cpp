...
namespace sdds {
   ...
   bool Date::operator==(const Date& newDate)const {
      bool eq = false;
      if (this->daysSince0001_1_1() == newDate.daysSince0001_1_1())
         eq = true;
      return eq;
   };
   bool Date::operator!=(const Date& newDate)const {
   ...
   int Date::operator-(const Date& newDate) const {
      int res;
      res = this->daysSince0001_1_1() - newDate.daysSince0001_1_1();
      return res;
   };
   Date::operator bool() {
      return validate();
   }
   ostream& operator<<(std::ostream& os, const Date& newDate) {
      newDate.write(os);
      return os;
   };
   istream& operator>>(std::istream& is, Date& newDate) {
      newDate.read(is);
      return is;
   };
}
